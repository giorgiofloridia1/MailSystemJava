...
public class Server {
        ...
        for (MailBox mailbox : account) {
            ...
            try {
                synchronized (fileNames.get(i)){
                    File emailFile = new File("C:\\Users\\giorg\\OneDrive\\Desktop\\Progetto ProgIII\\MailServer\\incoming_emails\\" + fileNames.get(i) + ".txt");
                    if(emailFile.exists()){
    ...
    public void writeToFile(String e, Email email) throws FileNotFoundException {
        ...
        synchronized (fileNames.get(i)){
            PrintWriter writer = new PrintWriter(new FileOutputStream("C:\\Users\\giorg\\OneDrive\\Desktop\\Progetto ProgIII\\MailServer\\incoming_emails\\" + fileNames.get(i) + ".txt", true));
